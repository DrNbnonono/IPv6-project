...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    const uploadDir = DIRECTORIES.uploads;
...
exports.uploadFile = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      switch(toolType) {
        case 'xmap':
          targetDir = DIRECTORIES.whitelists;
          filePrefix = 'whitelist_';
          break;
        case 'database':
          targetDir = DIRECTORIES.ipv6Addresses;
          filePrefix = 'address-';
          break;
        case 'zgrab2':
          if (fileType === 'config') {
            targetDir = DIRECTORIES.zgrab2Configs;
            filePrefix = 'zgrab2_config_';
          } else {
            targetDir = DIRECTORIES.zgrab2Inputs;
            filePrefix = 'zgrab2_input_';
          }
          break;
        case 'jsonanalysis':
          if (fileType === 'result') {
            targetDir = DIRECTORIES.jsonanalysis;
      ...
      try {
        ...
        if (renameError.code === 'EXDEV') {
          // 跨设备错误，使用复制+删除
          console.log('跨设备移动，使用复制方式...');
          fs.copyFileSync(req.file.path, filePath);
          fs.unlinkSync(req.file.path);
          console.log('文件复制移动成功');
        } else {
          throw renameError;
        }
      }
