...
const loadData = async () => {
  try {
    ...
    console.log('所有协议组件基础数据加载完成');
    loading.value = false;
  } catch (error) {
    console.error('加载协议组件数据失败:', error);
    loading.value = false;
  }
};
...
const handleUpdateProtocols = async () => {
  ...
  try {
    ...
    console.log('发送协议更新请求:', requestData);
